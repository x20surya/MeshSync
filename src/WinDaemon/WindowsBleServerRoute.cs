...
namespace WinDaemon
{
    ...
    ///
    /// <para>Thin, because <see cref="WindowsBleTransport"/> already holds the link. What this
    /// adds is the state machine: a link that has not agreed a session sits in
    /// <see cref="RouteState.Handshaking"/> and is closed on the shared deadline.</para>
    ///
    /// <para><b>That is the fix for this half of the defect.</b> The server used to return from a
    /// failed key agreement leaving the subscriber attached and <c>_hasSubscriber</c> true - so a
    /// device from another mesh subscribing to this machine's GATT server both looked connected
    /// and, because the central scan loop was gated on "is anything connected", stopped this
    /// machine looking for its own peers.</para>
    /// </summary>
    public sealed class WindowsBleServerRoute : IPeerRoute
    {
        private readonly WindowsBleTransport _server;
        ...
        public WindowsBleServerRoute(WindowsBleTransport server, ILinkClock? clock = null)
        {
            _server = server;
        ...
        public event Action<WindowsBleServerRoute, PeerIdentifiedEventArgs>? Identified;
        ...
        private void OnClosed(object? sender, EventArgs e) => Fail("the central unsubscribed");
        ...
        public async Task<bool> SendAsync(byte contentType, byte[] body, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                // Addressed rather than broadcast: notifying the characteristic reaches every
                // subscriber, and each payload is sealed for exactly one.
                await _server.SendPayloadToAsync(PeerFingerprint, payload, cancellationToken).ConfigureAwait(false);
                return true;
            }
            catch (Exception ex)
            {
        ...
        public Task<bool> RequestWiFiAsync() => _server.RequestWiFiAsync(PeerFingerprint);

        /// <summary>Runs the server's liveness check, which has no loop of its own.</summary>
