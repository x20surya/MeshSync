...
{
    /// <summary>
    /// The link a peer opened to this phone's advertised service, wearing the route interface.
    ///
    /// <para>Thin, because <see cref="AndroidBlePeripheral"/> already holds the link. What this
    /// adds is the state machine: a link that has not agreed a session sits in
    /// <see cref="RouteState.Handshaking"/> and is closed on the shared deadline rather than
    /// being reported as connected the moment a central subscribes.</para>
    /// </summary>
    public sealed class AndroidBlePeripheralRoute : IPeerRoute
    {
        private readonly AndroidBlePeripheral _server;
        ...
        public AndroidBlePeripheralRoute(AndroidBlePeripheral server, ILinkClock? clock = null)
        {
            _server = server;
        ...
        public CoreLib.Identity.PeerSession? Session => _server.Peer;
        ...
        public event Action<AndroidBlePeripheralRoute, PeerIdentifiedEventArgs>? Identified;

        private void OnIdentified(object? sender, PeerIdentifiedEventArgs e)
        {
            try { Identified?.Invoke(this, e); }
            catch (Exception ex) { Log.Write("BlePeripheral", "An Identified handler threw", ex); }
        ...
        private void OnPayload(object? sender, PayloadReceivedEventArgs e)
        {
            ...
            if (session == null)
            {
                Log.Write("BlePeripheral", "Dropped a payload that arrived before a key was agreed.");
                return;
            }

            if (!session.TryDecrypt(e.EncryptedPayload, out var decrypted))
            {
                Log.Write("BlePeripheral", "Dropped a payload that does not authenticate under this link's key.");
                return;
            }
            ...
            catch (Exception ex) { Log.Write("BlePeripheral", "Payload handling failed", ex); }
        }
        ...
        public async Task<bool> SendAsync(byte contentType, byte[] body, CancellationToken cancellationToken = default)
        {
            ...
            {
                Log.Write("BlePeripheral", $"{payload.Length} bytes is over the Bluetooth ceiling; Wi-Fi is needed.");
                return false;
            }

            try
            {
                await _server.SendPayloadAsync(payload, cancellationToken).ConfigureAwait(false);
                return true;
            }
            catch (Exception ex)
            {
                Log.Write("BlePeripheral", "Sending over Bluetooth failed", ex);
                return false;
            }
        }
        ...
        public Task CloseAsync(string reason)
        {
            ...
            try { _ = _server.DisconnectAsync(); }
            catch (Exception ex) { Log.Write("BlePeripheral", "Dropping the inbound link failed", ex); }
        ...
        private void Move(RouteState to)
        {
            ...
            catch (Exception ex) { Log.Write("BlePeripheral", "A StateChanged handler threw", ex); }
        }
